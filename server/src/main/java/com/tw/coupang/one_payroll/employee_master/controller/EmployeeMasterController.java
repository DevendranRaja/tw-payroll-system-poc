...
public class EmployeeMasterController {
    ...
    public ResponseEntity<EmployeeMaster> createEmployee(@Valid @RequestBody CreateEmployeeRequest request) {
        EmployeeMaster created = employeeMasterService.createEmployee(request);
        return ResponseEntity.status(HttpStatus.CREATED).body(created);
    }

    @PutMapping("/employee/{employeeId}")
    public ResponseEntity<EmployeeMaster> updateEmployee(@PathVariable String employeeId,
    ...
    public ResponseEntity<List<EmployeeMaster>> getEmployeesByDepartment(
